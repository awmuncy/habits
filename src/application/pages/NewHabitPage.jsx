...
class C_NewHabitPage extends Component {
  ...
  submitNewHabit(e) {
    ...
    let myForm = document.getElementById('new_habit');
    ...
    let habit_id = this.props.createHabit(habit);
  ...
  render() {
    if (this.state.closed) {
      let directTo = `/habit/${this.state.closed}`;
...
function Nevermind(props) {
  ...
  return (<span className='nevermind' onClick={e=>history.goBack()}>Cancel</span>);
}
...
let NewHabitPage = connect(connectors.props, connectors.dispatches)(C_NewHabitPage);
