node {
    ...
    stage ('Deploy') {
        ...
        withCredentials([
            ...
            sh """
                ssh -T -oStrictHostKeyChecking=no -i $KEY_FILE $USER@142.93.187.75 "docker login -p '${HUB_PASSWORD}' -u '${HUB_USER}' && docker run -d -p 5000:5000 awmuncy/habits:${env.BUILD_NUMBER}"
            """
        }
    }
}